...
# flag = {
#     '0xcdd1b25d', '0xa21a9280',
#     '0x0175b1c4', '0xd450e04c'
# }
...
if __name__ == '__main__':
    ...
    src, dst = ChainEnum.ETH, ChainEnum.Polygon
    spider_net = SpiderNetEnum.Polygon
    res_report = {
        'src': f"{src.value}",
        'dst': f"{dst.value}",
        'bridge': f"{cur_bridge}",
    ...
    input_df = pd.read_csv(Config().VALIDATION_DIR + f"/{src.value}-{dst.value}/{cur_bridge}/input.csv").iloc[:, 1].str.slice(0, 10)
    flag = set([e.lower() for e in input_df.values.flatten()])
    flag.add("deprecated")
    print(flag)
    ...
    for bridge in BridgeEnum.__members__.values():
        ...
        for idx, item in enumerate(tqdm(sample)):
            ...
            try:
                ...
                # dst_txs['input'].fillna('', inplace=True)
                dst_txs['input'] = dst_txs['input'].str.lower()
                dst_txs = dst_txs[dst_txs['input'].str.contains('|'.join(flag))]
            ...
            if res[0]['srcTxHash'] in y_true and y_true[res[0]['srcTxHash']] == res[0]['dstTxHash']:
                hit_num += 1
            # elif len(tmp_res) > 1:
            #     res['input'] = res['input'].str.lower()
            #     res = res[res['input'].str.contains('|'.join(flag))].to_dict('records')
            #     if len(res) == 1 and res[0]['srcTxHash'] in y_true and y_true[res[0]['srcTxHash']] == res[0]['dstTxHash']:
            #         hit_num += 1
