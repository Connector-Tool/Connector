...
from BlockchainSpider.items import BlockMetaItem, ExternalTransactionItem, InternalTransactionItem
from BlockchainSpider.items import ERCTokenItem, ERC20TokenTransferItem, ERC721TokenTransferItem, \
    ERC1155TokenTransferItem, BlockNumberItem, LogItem
from BlockchainSpider.utils.enum import ETHDataTypes


class MetaWeb3BlocksSpider(scrapy.Spider):
    name = 'blocks.meta.web3'
    custom_settings = {
        'ITEM_PIPELINES': {
            'BlockchainSpider.pipelines.BlockPipeline': 298,
            'BlockchainSpider.pipelines.BlockNumberPipeline': 299,
            **getattr(settings, 'ITEM_PIPELINES', dict())
        },
        'PROVIDERS_BUCKET': 'BlockchainSpider.utils.bucket.StaticProvidersBucket',
    }
    ...
    def __init__(self, **kwargs):
        ...
        # provider settings
        provider_bucket = self.__class__.custom_settings['PROVIDERS_BUCKET']
    ...
    def start_requests(self):
        if self.pending:
            yield from self.gen_pending_requests()
    ...
    async def parse_eth_get_block_by_number(self, response: scrapy.http.Response, **kwargs):
        async def _parse_eth_get_block_by_number(response: scrapy.http.Response, **kwargs):
            ...
            if has_internal_txs and ETHDataTypes.TRACE.value in self.data_types:
    ...
    def handle_response_error(self, response: scrapy.http.Response, func_name: str, cb_kwargs: dict):
        ...
        if cb_kwargs.get(retry_key, 0) >= self.max_retry:
            self.log(
                ...
                ) + "please check that whether the provider is available.",
                level=logging.ERROR
            )
            return
    ...
    def get_request_eth_get_block_by_number(
            self, block_number: Union[int, str], priority: int, cb_kwargs: dict
    ) -> scrapy.Request:
        return scrapy.Request(
            url=self.provider_bucket.get(),
            method='POST',
    ...
    async def get_erc20_or_erc721_info(self, contract_address: str) -> dict:
        ...
        # see https://stackoverflow.com/questions/45364197/how-to-detect-if-an-ethereum-address-is-an-erc20-token-contract
        results = await self._gather_eth_call_result(
            tx_objs=[{
                'to': contract_address,
        ...
        if results[0] is not None and results[0][0] is True:
            return {
                'address': contract_address,
                'is_erc20': False,
                'is_erc721': True,
        ...
        # detected ERC20, return if contract is ERC20
        # fetch decimals of erc20
        decimals = None
        for result in await self._gather_eth_call_result(
                tx_objs=[
        ...
        if decimals is not None:
            ...
            total_supply = results[0][0] if results[0] is not None else None
    ...
    async def get_created_contract_address(self, txhash: str) -> dict:
        ...
        try:
            ...
            self.log(
                message="Failed on 'eth_getTransactionReceipt', " +
                        "please check the providers are available, " +
                        "callback args {}.".format(str(req_data)),
                level=logging.WARNING
            )
        finally:
            await client.close()
    ...
    def split_to_words(data: str) -> list:
        if data and len(data) > 2:
            ...
            words = list(MetaWeb3BlocksSpider.chunk_string(data_without_0x, 64))
